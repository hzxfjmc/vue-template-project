<template lang="pug">
    .customer-info(v-show="!hideCustomer")
        .business-title(v-if="source && source !== 1")  已为您分配专属客户经理，即刻尊享1v1贴心服务
        .business-tip 保存图片，微信扫描二维码，即可添加客户经理为好友
        .businessCardWrap
    ...
    methods: {
        ...
        async initData() {
            try {
                ...
                this.setShowType(this.customerInfo)
            } catch (e) {
        ...
        // 设置展示类型，是否有完整客户经理信息
        setShowType(data) {
            delete data.licenseNum
            let vals = Object.values(data)
            if (vals.length === 0 || vals.some(item => !item)) {
                this.$emit('setShowType', 2) // 不展示客户经理信息
            } else {
                this.$emit('setShowType', 1) // 展示客户经理信息
            }
        },
        // 将html转为canvas
        downCustomerCard() {
