...
export default {
    ...
    async activated() {
        const infoArr = (
            LS.get('us-stock-market-statement-preview-pdf-info') || ''
        ).split(',')
        const {
            url = infoArr[0] || '',
            title = infoArr[1] || ''
        ...
        this.pages = 1 // 默认为1不让页面显示空

        try {
            const data = await getCosUrl(url)

            console.log('getCosUrl data:', data)

            viewport.set()
            data && this.loadFile(data)
            title && this.$html.setTitle(title)
        } catch (e) {
            this.pages = 0
            console.log('getCosUrl error:', e)
        }
    },
    deactivated() {
        this.pdfDoc = null
