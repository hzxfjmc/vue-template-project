    ...
    .section(:class="isRegister ? 'allGet':''")
        header
            .top
                ...
                a(@click='show=true') 活动规则
            .prize(v-if="!isRegister")
...
export default {
    ...
    created() {
        // 获取用户邀请码和活动id
        this.invitationCode = getParameter('ICode')
        this.activityId = getParameter('id')
        this.activityId &&
            this.$store.dispatch('setActivityId', this.activityId)
        this.invitationCode &&
        ...
        this.activityId && this.getBroadcast()
    },
    computed: {
        ...mapGetters(['userInfo'])
    },
    ...
    methods: {
        ...
        async getBroadcast() {
            try {
                this.broadcastData = await getBroadcastData('1009')
            } catch (e) {
        ...
        async getUserInfo() {
            try {
                ...
                if (getParameter('nickname')) {
                    this.isRegister = true
                    this.nickname = getParameter('nickname')
                } else {
